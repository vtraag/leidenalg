...
:class:`~leiden.VertexPartition.MutableVertexPartition`. In addition,
...
not immediately available in :func:`leiden.find_partition`.
"""
...
try:
    _dist = get_distribution('leiden')
    # Normalize case for Windows systems
    dist_loc = os.path.normcase(_dist.location)
    here = os.path.normcase(__file__)
    if not here.startswith(os.path.join(dist_loc, 'leiden')):
